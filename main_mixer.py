...
class Image():
    def __init__(self , img_array):
        ...
        self.real_comp[self.real_comp <= 0] = 10 ** -100 #111 leh
        ...
        # def mix(self , comp1 , comp2 , ratio , mode):
        #     comp1 = comp1 * rato[0] + comp2 * (1-ratio[0])
        #     comp2 = comp2 * rato[1] + comp1 * (1-ratio[1])
        #     if mode == "mag-phase":
        #         output = np.multiply(comp1 , np.exp(comp2 * 1j))
        #     elif mode == "real-imag":
        #         output = np.add(comp1 , comp2 * 1j)
        #     return np.real(ifft2(output))
...
class MainWindow(QtWidgets.QMainWindow , image_mixer.Ui_MainWindow):
    ...
    def mixing(self):
        if self.comboBox_5.currentText() == 'Mag' :
            ...
            self.available_items([False,False,False,False,True,False])
        ...
        #print(self.ratio[0] , self.ratio[1])
